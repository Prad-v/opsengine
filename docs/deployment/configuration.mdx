...
|       **KEEP_DEFAULT_USERNAME**       |        Default username for the admin user (DB auth only)         |    No    |    "admin"    |             Any valid username string              |
|       **KEEP_DEFAULT_PASSWORD**       |        Default password for the admin user (DB auth only)         |    No    |    "admin"    |             Any strong password string             |
...
|    **OPEN_AI_API_KEY**      |     Alias for `OPENAI_API_KEY` (frontend CopilotKit)     |    No    |        None         |                     Valid OpenAI API key                     |     Frontend     |
...
<Tip>
  Workflow Builder AI, incident chat, and related UI features require an OpenAI
  API key either via **Settings → AI** or `OPENAI_API_KEY` /
  `OPEN_AI_API_KEY` on the frontend. Without it, the UI shows "AI is disabled".
  See [AI Workflow Builder Assistant](/overview/ai-workflow-assistant).
</Tip>
...
<Info>
  Maintenance window rules suppress matching alerts during planned work.
  New rules keep alerts in the feed as suppressed unless you choose hide-from-feed.
  `recover_previous_status` starts the watcher automatically so alerts do not stay in `maintenance` after the window ends.
</Info>

| Env var | Purpose | Required | Default Value | Valid options |
|---|---|---|---|---|
| **KEEP_MAINTENANCE_WINDOWS_ENABLED** | Apply maintenance rules when ingesting alerts | No | `true` | `true`, `false` |
| **MAINTENANCE_WINDOW_STRATEGY** | How matching alerts are handled | No | `default` | `default`, `recover_previous_status` |
| **KEEP_WATCHER_LAPSED_TIME** | Seconds between recover-strategy reviews | No | `60` | Positive integer |
| **WATCHER** | Start the background watcher | No | `false` | `true`, `false` |
| **MAINTENANCE_WINDOWS** | Also start the watcher (legacy) | No | `false` | `true`, `false` |
...
| **ALERT_SIDEBAR_FIELDS**           | Specifies a list of fields to show in the sidebar.                  | No       | service,source,code,description,message,fingerprint,url,incidents,timeline,relatedServices | Valid list of fields |
