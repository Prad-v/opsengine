...
On a local kind cluster this repo uses two namespaces: `keep` (local backend images + host UI) and `keep-prod` (published Helm / CI artifact images + in-cluster UI). See [Local kind](/deployment/kubernetes/kind-local).
