...
For a concrete Opsengine target of **1M events → 5K incidents → 5K workflows per day**, see the architecture overview and phased roadmap: [Scale to 1M events](/deployment/scale-1m-events-architecture).
