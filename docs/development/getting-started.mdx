...
Run the **API and UI on your machine** (Poetry venv + Node), and run **Postgres, Redis, Soketi, and Temporal in Docker**.
...
2. Start `docker-compose.deps.yml` + `docker-compose.temporal.yml` (Postgres `:5432`, Redis `:6379`, Soketi `:6001`, Temporal `:7233` / UI `:8233`)
...
With Keep running, start the mock alert-source UI to register providers and fire webhook events for e2e validation:

```
make mock-providers
```

Open [http://localhost:8099](http://localhost:8099). Full guide: [Provider mock](/development/provider-mock).
...
See also [Docker deployment](/deployment/docker) for pulling published images with `make prod`.
