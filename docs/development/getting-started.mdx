...
2. Start `docker-compose.deps.yml` + `docker-compose.temporal.yml` (Postgres `:5432`, Redis `:6379`, Soketi `:6001`, Temporal `:7233` / UI `:8233`) and the **provider-mock** UI (`:8099`)
3. Run `poetry run keep api` on `:8080`
4. Run `npm run dev` in `keep-ui` on `:3000`

> **UI note:** Incidents (`/incidents`) and Alerts Feed (`/alerts/feed`) use Material UI. See [Material UI](/development/material-ui).

| Process | Where it runs | URL / port |
| --- | --- | --- |
| UI | Host (npm) | http://localhost:3000 |
...
`make start` (and `make deps`) starts the provider-mock UI automatically at [http://localhost:8099](http://localhost:8099). Use it to register mock Grafana / Mimir / VictoriaMetrics providers and fire webhook events (including the **NVIDIA GPU / AI datacenter** pack). Full guide: [Provider mock](/development/provider-mock).
...
Grafana includes a **payments** correlation demo and an **NVIDIA GPU / AI datacenter** pack (DCGM-style H100 alerts). `make start` auto-wires the remediation loop (Temporal → mock GPU server → resolve or email). See [GPU remediate demo](/overview/demo-mock-nvidia-gpu-remediate). Re-run with `make demo-nvidia-gpu` if needed.
