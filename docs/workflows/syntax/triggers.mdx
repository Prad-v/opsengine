...
- Reserved alert code: `has(labels.code) && labels.code == "NVIDIA_GPU_THERMAL"`

Prefer **`labels.code`** over matching `name` or `description`. See [Alert codes](/overview/alert-code-catalog).
...
triggers:
  - type: incident
    events:
      - created
      - updated
    # Optional. Filter by reserved codes copied from member alerts.
    cel: code.startsWith("NVIDIA_GPU") || codes.exists(c, c.startsWith("NVIDIA_GPU"))
```

Incident CEL sees `code` (primary), `codes` (unique list), `name`, and other incident fields. A workflow may list **both** an `alert` trigger and an `incident` trigger.
