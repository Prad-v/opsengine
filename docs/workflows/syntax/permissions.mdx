...
3. If the `permissions` field is empty or not defined, any user with the `execute:workflows` permission can run the workflow

Set `require_approval: true` on the workflow (or add a `run_workflow` [approval policy](/overview/approvals)) to create a pending request instead of starting immediately. `permissions` still control who may **request** the run.
